...
import {
  ...
  BrainCircuit, FlaskConical, GitBranch, Download
} from 'lucide-react'
...
const PRESETS = [
  { value: 'very_light',              label: '🌱 Very Light',   desc: 'Fastest, minimal' },
  { value: 'light',                   label: '🪶 Light',        desc: 'Fast, small memory' },
  ...
  { value: 'best_quality',            label: '🥇 Best',         desc: 'Max accuracy' },
  { value: 'best_v150',               label: '🏆 Best v1.5',    desc: 'Optimized max' },
  { value: 'extreme',                 label: '⚡ Extreme',      desc: 'SOTA (<100k rows)' },
  { value: 'interpretable',           label: '🔍 Interpretable',desc: 'Glass-box only' },
  { value: 'optimize_for_deployment', label: '🚀 Deployment',   desc: 'Small & fast' },
]
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          <a
            href={`/api/download/model/${entry.run}?filename=${encodeURIComponent(label.replace(/[^a-zA-Z0-9_\-]/g, '_'))}_model`}
            download={`${label.replace(/[^a-zA-Z0-9_\-]/g, '_')}_model.zip`}
            onClick={e => e.stopPropagation()}
            title={`Download ${label} model ZIP`}
            style={{
              display: 'flex', alignItems: 'center', gap: 5,
              padding: '5px 12px', borderRadius: 7,
              background: 'var(--cyan-dim)', color: 'var(--cyan)',
              border: '1px solid var(--cyan)',
              fontSize: 11, fontWeight: 600, textDecoration: 'none', transition: 'all 0.15s',
              whiteSpace: 'nowrap',
            }}
...
export default function AutoML() {
  ...
  const [config, setConfig] = useState({
    problem_type: 'auto', eval_metric: 'auto',
    presets: 'medium_quality', time_limit: 300, holdout_frac: 0.2,
  })
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        setTarget(res.data.preview.columns[res.data.preview.columns.length - 1])
      } catch (err) {
  ...
  async function fetchStatus() {
    try {
      const res = await automlApi.status()
      const { training_status, leaderboards: lbs } = res.data
      if (lbs) setLeaderboards(lbs)
      if (training_status === 'training') {
        setTraining(true)
        startPolling()
      } else if (training_status === 'error' && lbs?.length === 0) {
        toast.error(`Training error: ${res.data.training_error}`)
      }
    } catch { /* no project yet, ignore */ }
  }
    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        if (training_status === 'done') {
          setTraining(false); stopPolling()
          toast.success('Training complete! 🎉')
          refreshProjects()
        } else if (training_status === 'error') {
          setTraining(false); stopPolling()
          toast.error(`Training failed: ${res.data.training_error}`)
          refreshProjects()
        }
      } catch { stopPolling(); setTraining(false) }
    }, 4000)
  }
  ...
  async function cancelTraining() {
    try { await automlApi.cancel(); setTraining(false); stopPolling(); toast.success('Training cancelled.') }
    ...
    <PageTransition key="automl">
      <div className="page-header">
        ...
        <p className="page-subtitle">
          Configure AutoGluon and launch training. Each run produces a full leaderboard with feature importance.
        </p>
      </div>
      ...
      <div className="grid-2" style={{ gap: 24, marginBottom: 32, alignItems: 'start' }}>
        ...
        <div className="card">
          ...
          <div className="form-group">
            ...
            <input type="text" className="form-input" value={runLabel}
              ...
              placeholder={`Run ${leaderboards.length + 1}`} />
          ...
          <div className="form-group">
            <label className="form-label">
              <Target size={11} style={{ marginRight: 4, verticalAlign: 'middle' }} />Target Column
            </label>
            <select className="form-select" value={target} onChange={e => setTarget(e.target.value)}
              style={{ borderColor: validErrors.target_column ? 'var(--red)' : undefined }}>
              {columns.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            {validErrors.target_column && <div style={{ fontSize: 11, color: 'var(--red)', marginTop: 4 }}>{validErrors.target_column}</div>}
          </div>
          ...
          <div className="form-group">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {PROBLEM_TYPES.map(pt => (
                <motion.div key={pt.value} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                  onClick={() => setCfg('problem_type', pt.value)}
                  style={{ padding: '10px 12px', borderRadius: 8, cursor: 'pointer', transition: 'all 0.15s',
          ...
          <div className="form-group">
            ...
            <select className="form-select" value={config.eval_metric} onChange={e => setCfg('eval_metric', e.target.value)}>
          ...
          <div className="form-group">
            ...
            <select className="form-select" value={config.presets} onChange={e => setCfg('presets', e.target.value)}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div className="card">
            ...
            <div style={{ fontSize: 12, color: 'var(--text-3)' }}>Increase to 1800–3600 s for Best Quality or Extreme.</div>
          </div>
          ...
          {leaderboards.length > 0 && (
            <div className="card" style={{ padding: '14px 16px' }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-2)' }}>
                Change config and click "Train New Run" to add another leaderboard for comparison.
              </div>
            </div>
          )}
          ...
          {!training ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <motion.button className="btn btn-primary btn-full btn-lg" disabled
                style={{ position: 'relative', overflow: 'hidden' }}>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 6 }}>
                Runs in the background — you can switch projects or navigate freely.
              </div>
            </div>
